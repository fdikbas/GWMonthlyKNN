...
full_network_masked_gap_metrics.csv
    Detailed station-level validation metric results.
    It includes one row per method, station, and gap length.

full_network_masked_gap_predictions.csv
    Detailed month-level observed-versus-predicted values for every masked validation
    month. This file is the best output for auditing individual predictions. It includes
    one row per method, station, gap length, and masked month, with observed_WSE,
    predicted_WSE, residual, and window information.
...
Finally, inspect full_network_masked_gap_predictions.csv if you want to audit the
actual hidden observations and reconstructed values month by month. This is especially
useful for checking whether one method performs better only in selected donor-rich
situations, or whether a method has systematic high/low residuals in certain wells.
...
def append_prediction_rows(prediction_rows, method, variant, st, grp, L, start, end,
                           target_dates, true_vals, pred_vals, k=np.nan, w=np.nan,
                           weights="", note=""):
    """
    Append month-level observed-versus-predicted values for one masked-gap test.

    This function does not change the validation metrics. It only records the actual
    hidden values and the corresponding predictions so that users can audit the
    validation output at monthly resolution.

    Output columns written later to full_network_masked_gap_predictions.csv:
    - method, variant, k, w, weights
    - STATION, GROUP, gap_len
    - window start/end dates
    - MSMT_DATE for each masked month
    - observed_WSE, predicted_WSE, residual = predicted_WSE - observed_WSE
    - note for cases such as failed donor regression
    """
    true_arr = np.asarray(true_vals, dtype=float)

    if pred_vals is None:
        pred_arr = np.full(len(true_arr), np.nan, dtype=float)
    else:
        pred_arr = np.asarray(pred_vals, dtype=float)

    # Safety: keep lengths aligned even if a baseline unexpectedly returns a different length.
    n = min(len(target_dates), len(true_arr), len(pred_arr))
    for idx in range(n):
        obs = true_arr[idx]
        pred = pred_arr[idx]
        residual = pred - obs if np.isfinite(obs) and np.isfinite(pred) else np.nan
...
# ----------------------------
results = []
prediction_rows = []  # Month-level observed-versus-predicted validation records.
...
for row in tqdm(valid_windows.itertuples(index=False), total=len(valid_windows), desc="Evaluating stations", unit="test"):
    ...
    if RUN_BASELINES:
        ...
        append_prediction_rows(
            prediction_rows,
            method="baseline_seasonal_climatology",
            variant="",
            st=st,
            grp=grp,
            L=L,
        ...
        if pred_reg is not None:
            ...
            append_prediction_rows(
                prediction_rows,
                method="baseline_neighbor_regression",
                variant="",
                st=st,
                grp=grp,
                L=L,
            ...
            append_prediction_rows(
                ...
                true_vals=true_vals,
                pred_vals=None,
                note="no_valid_donor_or_overlap",
            )
            results.append(
                dict(method="baseline_neighbor_regression", variant="", STATION=st, GROUP=grp, gap_len=L,
...
if not pred_df.empty:
    # Keep date columns clean and readable in CSV outputs.
    for col in ["start_date", "end_date", "MSMT_DATE"]:
        pred_df[col] = pd.to_datetime(pred_df[col]).dt.strftime("%Y-%m-%d")
pred_df.to_csv(os.path.join(OUTDIR, "full_network_masked_gap_predictions.csv"), index=False)
...
print("- full_network_masked_gap_predictions.csv")
